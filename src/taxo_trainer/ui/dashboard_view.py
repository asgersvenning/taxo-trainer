...
def render_dashboard_view() -> None:
    ...
    with container:
        ...
        with (
            ...
            ui.row().classes("w-full justify-between items-center flex-wrap gap-3"),
        ):
            ...
            with ui.row().classes(
                "items-center gap-1.5 bg-gray-900 p-1.5 rounded-lg border border-gray-700"
            ):
        ...
        def refresh_dashboard() -> None:
            ...
            best_fams, worst_fams = get_family_mastery_stats(
                user_conn, app_conn, time_range=r_val, limit=4
            )
            trouble_taxa = get_trouble_taxa(
                user_conn, app_conn, time_range=r_val, limit=5
            )
            confusion = get_confusion_matrix(
                user_conn, app_conn, time_range=r_val, limit=10
            )
            ...
            with content_container:
                ...
                with ui.row().classes("w-full gap-4 justify-between flex-wrap"):
                    ...
                    with ui.card().classes(
                        ...
                        ui.label("Current streak | All-time record").classes(
                ...
                with ui.row().classes("w-full gap-4 justify-between flex-wrap"):
                    ...
                    with ui.card().classes(
                        "flex-1 min-w-[320px] bg-gray-800 p-4 rounded-lg shadow-md border border-gray-700 space-y-2"
                    ):
                        with ui.row().classes("items-center gap-2"):
                            ui.icon("verified", color="green-400").classes("text-base")
                            ui.label("Top Mastered Plant Families").classes(
                                "text-sm font-bold text-green-400"
                            )
                        if not best_fams:
                            ui.label("No family attempts recorded yet.").classes(
                            ...
                            with ui.column().classes("w-full gap-1.5"):
                                for f in best_fams:
                                    with ui.row().classes(
                                        ...
                                        ui.label(f.display_name).classes(
                                            "text-xs font-medium text-white truncate max-w-[200px]"
                                        )
                                        ui.label(
                                            f"{f.accuracy_pct}% ({f.correct_attempts}/{f.total_attempts})"
                    ...
                    with ui.card().classes(
                        ...
                        with ui.row().classes("items-center gap-2"):
                            ...
                            ui.label("Families Needing Practice").classes(
                                "text-sm font-bold text-amber-400"
                            )
                        if not worst_fams:
                            ui.label("No weak family patterns identified yet!").classes(
                                "text-xs text-gray-500 italic py-2"
                            )
                        else:
                            with ui.column().classes("w-full gap-1.5"):
